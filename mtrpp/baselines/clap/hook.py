...
from .module.factory import load_state_dict
# from .module.data import get_audio_features, int16_to_float32, float32_to_int16
...
class CLAP_Module(torch.nn.Module):
    ...
    def get_audio_backbone_features(self, x, use_tensor=False):
        self.model.eval()
        audio_input = []
        for audio_waveform in x:          
            # quantize
            ...
            audio_input.append(temp_dict)
        audio_embed = self.model.audio_backbone_features(audio_input)
        if not use_tensor:
            audio_embed = audio_embed.detach().cpu().numpy()
        return audio_embed
    ...
    def get_text_backbone_features(self, x, tokenizer = None, use_tensor = False):
        self.model.eval()
        if tokenizer is not None:
            text_input = tokenizer(x)
        else:
            text_input = self.tokenizer(x)
        text_embed = self.model.text_backbone_features(text_input)
        if not use_tensor:
            text_embed = text_embed.detach().cpu().numpy()
        return text_embed
