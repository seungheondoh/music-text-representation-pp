...
class KVT_Dataset(Dataset):
    ...
    def get_split(self):
        track_split = json.load(open(os.path.join(self.data_path, "kvt", "track_split.json"), "r"))
    ...
    def get_file_list(self):
        annotation = json.load(open(os.path.join(self.data_path, "kvt", "annotation.json"), 'r'))
        self.list_of_label = json.load(open(os.path.join(self.data_path, "kvt", "kvt_tags.json"), 'r'))
    ...
    def audio_load(self, path):
        audio_path = os.path.join(self.data_path, "kvt", "audio", path)
        audio, _ = load_audio(
            path=audio_path,
    ...
    def __getitem__(self, index):
        ...
        audio = self.audio_load(track_id + ".mp3")
        return {
            "audio":audio, 
            "track_id":track_id, 
            "tags":tags, 
