...
class MTAT_Dataset(Dataset):
    ...
    def get_split(self):
        track_split = json.load(open(os.path.join(self.data_path, "mtat", f"{self.split_type}_track_split.json"), "r"))
    ...
    def get_file_list(self):
        annotation = json.load(open(os.path.join(self.data_path, "mtat", f"{self.split_type}_annotation.json"), 'r'))
        self.list_of_label = json.load(open(os.path.join(self.data_path, "mtat", f"{self.split_type}_mtat_tags.json"), 'r'))
    ...
    def audio_load(self, path):
        audio_path = os.path.join(self.data_path, "mtat", "audio", path)
        audio, _ = load_audio(
            path=audio_path,
    ...
    def __getitem__(self, index):
        ...
        audio = self.audio_load(audio_path)
        return {
            "audio":audio, 
            "track_id":track_id, 
            "tags":tags, 
